...
class GenerateAgentsMdTests(unittest.TestCase):
    def test_generates_agents_for_django_project(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Planejar slices enxutos: tocar poucos arquivos (ideal <= 5) e so o necessario para entregar valor.", content)
            self.assertIn("Cada slice deve incluir handoff + prompt pronto para implementador LLM com contexto zero.", content)
            self.assertIn("Cada slice deve herdar e preservar o Contract Freeze de design.md.", content)
            self.assertIn("Leis permanentes do repositorio: openspec/project.md", content)
            self.assertIn("Principios de engenharia para agentes LLM: docs/foundations/llm-engineering-principles.md", content)
            self.assertIn("No REFACTOR, preservar as leis do repositorio em openspec/project.md.", content)
            self.assertIn("design.md com Contract Freeze e obrigatorio, exceto QUICK de bugfix simples e reversivel.", content)
            self.assertIn("Gerar relatorio conciso e diff-oriented do slice em markdown temporario.", content)
            self.assertIn("Nao aprovar o proprio trabalho; reviewer/planner gate e separado.", content)
            self.assertIn("REPORT_PATH informado para avaliacao do planner", content)
