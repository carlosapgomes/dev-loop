...
7. `artifacts-consistency-checker`
...
8. `release-evidence-pack-generator`
...
2. `esaa-validate-agents`
```bash
/esaa-validate-agents [arquivos...] [--fix]
```

3. `esaa-django-insights`
```bash
...
5. `esaa-generate-agents`, `esaa-generate-context`, `esaa-classify-risk`
...
openspec init
mkdir -p docs/adr docs/releases prompts scripts tests/unit tests/integration
```
...
2. Criar change (`/opsx:propose` ou manual).
3. Produzir artefatos conforme risco.
4. Implementar 1-2 slices no maximo.
5. Rodar gate:
```bash
...
2. gerar ADR pendente (se houver decisao estrutural)
```bash
python3 esaa-deepseek/skills/adr-generator/adr_generator.py --project-root . --title "Decisao de release X"
```

3. atualizar changelog (processo do skill)
4. gerar `docs/releases/YYYY-MM-DD_vX.Y.Z.md` (template release evidence pack)
