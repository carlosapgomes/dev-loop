...
```
$ /esaa-suggest-adr migrate-to-postgres
...
1. Criar ADR automaticamente:
   /esaa-suggest-adr migrate-to-postgres --auto-create
...
```
$ /esaa-suggest-adr fix-typo-navbar
...
```
$ /esaa-suggest-adr adopt-celery-tasks
...
```
$ /esaa-suggest-adr auth-jwt-implementation --auto-create
...
def main():
    ...
    if args.auto_create:
        ...
        print("\n💾 Ações Disponíveis:")
        print(f"\n1. Criar ADR automaticamente:")
        print(f"   /esaa-suggest-adr {change_id} --auto-create")
        print(f"\n2. Criar manualmente:")
...
/esaa-suggest-adr add-feature --auto-create
```
...
/esaa-suggest-adr choose-database --auto-create
...
/esaa-suggest-adr implement-jwt-auth
# Mostra template completo de auth

# Revisa template, decide criar
/esaa-suggest-adr implement-jwt-auth --auto-create
...
cp -r esaa-suggest-adr ~/.pi/skills/

# 2. Dependências
pip install pyyaml

# 3. Disponível como /esaa-suggest-adr
```
...
| Skill | Quando Usar | Relação com esaa-suggest-adr |
|-------|-------------|------------------------------|
| `esaa-classify-risk` | Antes | Detecta se é HIGH/ARCH |
| `esaa-suggest-adr` | Durante | Cria documentação da decisão |
